...
class ETSsmModel(TsModel):
    """
    Theta model with statsmodels implementation

    https://www.statsmodels.org/dev/examples/notebooks/generated/ets.html#Simple-exponential-smoothing
    https://otexts.com/fpp3/ets-estimation.html
    ...
    def __init__(self, data, params, **kwargs):
        ...
        self.set_params()
        self.model: ETSModel = None
        self.model_fit: ETSResults = None
    ...
    def fit(self):
        ...
        if self.params['seasonal'] is not None:
            bounds['smoothing_seasonal'] = (self.params['smoothing_seasonal_min'], self.params['smoothing_seasonal_max'])
    ...
    def trial_params_full():
        params_trial = ETSsmModel.trial_params(damp=False)
    ...
    @staticmethod
    def names_params():
        return [p['name'] for p in ETSsmModel.trial_params_full()]

    @staticmethod
    def trial_params_grid(trend=True, seasonal=True, multiplicative=True, level=True, damp=False):
        trial_params_ = ETSsmModel.trial_params(trend, seasonal, multiplicative, level, damp)
        return TsModel.trial_params_grid(trial_params_)

    def flexibility(self):
        flexibility = 0
        flexibility += self.model_fit.smoothing_level ** 2

        if self.model_fit.model.has_trend:
            if self.model_fit.model.damped_trend is False:
                not_damped_trend = 1
            else:
                not_damped_trend = self.model_fit.damping_trend ** 2

            flexibility += not_damped_trend

            if self.model_fit.model.trend == 'add':
                flexibility += (1 + not_damped_trend) * self.model_fit.smoothing_trend ** 2
            else:
                flexibility += (2 + not_damped_trend) * self.model_fit.smoothing_trend ** 2

        if self.model_fit.model.has_seasonal:
            if self.model_fit.model.seasonal == 'add':
                flexibility += self.model_fit.smoothing_seasonal ** 2
            else:
                flexibility += 2 * self.model_fit.smoothing_seasonal ** 2
