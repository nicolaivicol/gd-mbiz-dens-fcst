...
class TestHoltWintersSmModel(TestModel):
    ...
    def test_general(self):
        self.general(HoltWintersSmModel)

    def test_damping(self):
        params_more_damping = {'seasonal': 'no', 'damped_trend': True, 'damping_trend_min': 0.80, 'damping_trend_max': 0.85}
        m_more_damping = HoltWintersSmModel(self.ts_in, params=params_more_damping)
        m_more_damping.fit()
        ...
        m_less_damping = HoltWintersSmModel(self.ts_in, params=params_less_damping)
        m_less_damping.fit()
        f_less_damping = m_less_damping.predict(12)

        assert 0.97 <= m_less_damping.model.params['damping_trend'] <= 0.99
        assert 0.80 <= m_more_damping.model.params['damping_trend'] <= 0.85
